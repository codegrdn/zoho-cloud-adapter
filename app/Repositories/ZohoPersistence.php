...
class ZohoPersistence implements ZohoPersistenceInterface
{
    public function saveOAuthData($zohoOAuthTokens)
    {
        ...
        try {
            ...
            Logger::severe("Exception occured while inserting OAuthTokens into DB(file::ZohoPersistence)({$ex->getMessage()})\n{$ex}");
        } finally {
            if ($db_link != null) {
                $db_link->close();
            }
        ...
        try {
            ...
            if (! $resultSet) {
                ...
                while ($row = mysqli_fetch_row($resultSet)) {
                    $oAuthTokens->setExpiryTime($row[4]);
                    $oAuthTokens->setRefreshToken($row[3]);
                    $oAuthTokens->setAccessToken($row[2]);
                    $oAuthTokens->setUserEmailId($row[1]);
                    break;
                }
            }
        } catch (Exception $ex) {
            Logger::severe("Exception occured while getting OAuthTokens from DB(file::ZohoPersistence)({$ex->getMessage()})\n{$ex}");
        } finally {
            if ($db_link != null) {
                $db_link->close();
            }
    ...
    public function deleteOAuthTokens($userEmailId)
    {
        ...
        try {
            ...
            Logger::severe("Exception occured while Deleting OAuthTokens from DB(file::ZohoPersistence)({$ex->getMessage()})\n{$ex}");
        } finally {
            if ($db_link != null) {
                $db_link->close();
            }
    ...
    {
        $mysqli_con = new \mysqli(env('DB_HOST').":". env('DB_PORT'), env('DB_USERNAME'), env('DB_PASSWORD'), env('DB_DATABASE'));
